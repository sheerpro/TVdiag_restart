...
class EventProcess():
    ...
    def process(self, reconstruct=False):
        ...
        # label_path = f"/home/fuxian/lky/TVDiag_new_gaia/extractor/GAIA/allgroundtruth.csv"
        # metric_path = f"/home/fuxian/lky/TVDiag_new_gaia/extractor/GAIA/events/metric.json"
        # trace_path = f"/home/fuxian/lky/TVDiag_new_gaia/extractor/GAIA/events/trace.json"
        # log_path = f"/home/fuxian/lky/TVDiag_new_gaia/extractor/GAIA/events/log.json"
        # edge_path = f"/home/fuxian/lky/TVDiag_new_gaia/extractor/GAIA/events/edges.json"
        # node_path = f"/home/fuxian/lky/TVDiag_new_gaia/extractor/GAIA/events/nodes.json"

        # label_path = f"/home/fuxian/lky/TVDiag_new_aiops/extractor/AIOPS/allgroundtruth.csv"
        # metric_path = f"/home/fuxian/lky/TVDiag_new_aiops/extractor/AIOPS/events/metric.json"
        # trace_path = f"/home/fuxian/lky/TVDiag_new_aiops/extractor/AIOPS/events/trace.json"
        # log_path = f"/home/fuxian/lky/TVDiag_new_aiops/extractor/AIOPS/events/log.json"
        # edge_path = f"/home/fuxian/lky/TVDiag_new_aiops/extractor/AIOPS/events/edges.json"
        # node_path = f"/home/fuxian/lky/TVDiag_new_aiops/extractor/AIOPS/events/nodes.json"

        # label_path = f"/home/fuxian/DataSet/NewDataset/tt/allgroundtruth.csv"
        # metric_path = f"/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/events/metric.json"
        # trace_path = f"/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/events/trace.json"
        # log_path = f"/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/events/log.json"
        # edge_path = f"/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/events/edges.json"
        # node_path = f"/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/events/nodes.json"

        label_path = f"/home/fuxian/DataSet/NewDataset/ob/allgroundtruth.csv"
        metric_path = f"/home/fuxian/lky/TVDiag_new_onlineboutique/extractor/onlineboutique/events/metric.json"
        trace_path = f"/home/fuxian/lky/TVDiag_new_onlineboutique/extractor/onlineboutique/events/trace.json"
        log_path = f"/home/fuxian/lky/TVDiag_new_onlineboutique/extractor/onlineboutique/events/log.json"
        edge_path = f"/home/fuxian/lky/TVDiag_new_onlineboutique/extractor/onlineboutique/events/edges.json"
        node_path = f"/home/fuxian/lky/TVDiag_new_onlineboutique/extractor/onlineboutique/events/nodes.json"
    ...
    def build_embedding(self):
        ...
        for key, data in data_map.items():
            ...
            for i, idx in enumerate(train_idxs):
                ...
                current_instance = train_ins_labels[i]
                current_type = train_type_labels[i]
                ...
                for node in nodes:
                    ...
                    if node == current_instance:
                        labels.append(f'__label__{node}{self.types.index(current_type)}')
                    else:
                        # 对于非故障节点，应该标记为正常类型而不是固定的0
                        labels.append(f'__label__{node}{self.types.index("normal")}')
                    # if key == 'metric':
                    #     print(f"labels: {labels}")
            # print(f"docs: {docs}")
            # print(f"labels: {labels}")
            ...
            for idx in self.labels['index']:
                ...
                for node in nodes:
                    ...
                    # print(f"Node: {node}, graph_Embedding: {graph_embs}")
                    # print(f"Graph embedding for {key} at index {idx}: {graph_embs}")
                embs[idx]=graph_embs
    ...
    def build_dataset(self):
        ...
        # 根据配置将特定模态置零
        if not self.config.use_metric:
            self.logger.info("Ablating metric modality - setting to zero")
            for key in metric_embs:
                metric_embs[key] = np.zeros_like(metric_embs[key])
        
        if not self.config.use_trace:
            self.logger.info("Ablating trace modality - setting to zero")
            for key in trace_embs:
                trace_embs[key] = np.zeros_like(trace_embs[key])
        
        if not self.config.use_log:
            self.logger.info("Ablating log modality - setting to zero")
            for key in log_embs:
                log_embs[key] = np.zeros_like(log_embs[key])
