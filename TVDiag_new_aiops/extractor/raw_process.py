...
def extract_traces(trace_df: pd.DataFrame, start_time):
    ...
    con1 = trace_df['st_time'] > start_time - 6*window
    con2 = trace_df['st_time'] < start_time
    con3 = trace_df['st_time'] > start_time
    con4 = trace_df['st_time'] < start_time + 2*window
...
def extract_logs(log_df: pd.DataFrame, start_time):
    ...
    con1 = log_df['timestamp'] > start_time - 6*window
    con2 = log_df['timestamp'] < start_time
    con3 = log_df['timestamp'] > start_time
    con4 = log_df['timestamp'] < start_time + 2*window
...
def extract_metrics(metric_df: pd.DataFrame, start_time):
    ...
    con1 = metric_df['timestamp'] > start_time - 6*window
    con2 = metric_df['timestamp'] < start_time
    con3 = metric_df['timestamp'] > start_time
    con4 = metric_df['timestamp'] < start_time + 2*window
...
if __name__ == '__main__':
    ...
    input_dir = "/home/fuxian/DataSet/NewDataset/aiops"
    ...
    label_df = pd.read_csv("/home/fuxian/lky/TVDiag_new_aiops/extractor/AIOPS/allgroundtruth.csv")
