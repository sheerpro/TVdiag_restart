...
class MultiModalDataSet(Dataset):
    def __init__(self, metrics, traces, logs, instance_labels, type_labels, nodes, edges):
        ...
        for i in range(len(instance_labels)):
            ...
            #metrics_i_expanded = np.tile(metrics[i], (4,1))  # 将 metrics[i] 重复 4 次
            #traces_i_expanded = np.tile(traces[i], (4,1))
            #logs_i_expanded = np.tile(logs[i], (4,1))
            #metrics[i] = np.array(metrics[i][:10])
