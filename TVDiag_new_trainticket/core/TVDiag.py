...
class TVDiag(object):
    ...
    def train(self, train_data, aug_data):
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_graphs, batch_labels in train_dl:
                ...
                # if self.config.dynamic_weight:
                #     total_loss = awl(l_rcl, l_fti, l_to, l_cm)
                # else:
                #     total_loss = l_rcl + l_fti + sigma * (l_to + l_cm)
    ...
    def evaluate(self, test_data, model=None):
        if model is None:
            ...
            print(os.path.join(self.writer.log_dir, 'TVDiag.pt'))
            model = MainModel(self.config).to(self.device)
        ...
        for data in test_data:
            ...
            # print(f"Evaluating graph with {graph.num_nodes()} nodes, failure type: {failure_type}")
            roots.append(graph.ndata['root'])
            # print("##########################3",graph.ndata.keys())  # 输出形如 [num_nodes, ft_num]
            # print(f"Root nodes: {graph.ndata['root']}")
        ...
        # print (f"root logits:{root_logits}")
        # print (f"num_node_list:{num_node_list}")
        # print (f"roots:{roots}")
        # print("12312312313",rcl_res)
