...
if __name__ == '__main__':
    ...
    input_dir = "/home/fuxian/DataSet/NewDataset/tt"
    output_dir = "/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket"
    ...
    label_df = pd.read_csv("/home/fuxian/DataSet/NewDataset/tt/allgroundtruth.csv")
    ...
    label_df['ed_time'] = label_df['ed_time'].apply(lambda x: time2stamp(str(x).split('.')[0]))
     ...
     # 保存处理后的 label_df
    processed_label_file = os.path.join(output_dir, "all_processed_labels.csv")
    label_df.to_csv(processed_label_file, index=False)
    print(f"处理后的标签数据已保存到: {processed_label_file}")
