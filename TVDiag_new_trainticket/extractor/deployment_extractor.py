...
failure_post_data: dict = io_util.load('/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/post-data-10.pkl')
# 将第一列设置为索引
...
for idx, row in tqdm(label_df.iterrows(), total=label_df.shape[0]):
    ...
    # 遍历所有日期目录
    base_dir = '/home/fuxian/DataSet/NewDataset/tt'
    for date_dir in sorted(os.listdir(base_dir)):
        # 检查是否是日期格式的目录
        if not os.path.isdir(os.path.join(base_dir, date_dir)) or not date_dir.startswith('2023-'):
            continue
...
io_util.save_json('/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/events/nodes.json', deployment_nodes)
io_util.save_json('/home/fuxian/lky/TVDiag_new_trainticket/extractor/trainticket/events/edges.json', deployment_edges)